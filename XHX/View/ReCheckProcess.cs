    ...
    public partial class ReCheckProcess : BaseForm
    {
        ...
        private void SearchAnswerLog()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int j = 0; j < ds.Tables[0].Rows.Count; j++)
                {
                    ...
                    answer.SpecialCaseChk = Convert.ToBoolean(ds.Tables[0].Rows[j]["SpecialCaseChk"]);
                    ...
                    else if (answer.StatusCode == "S3")
                    {
                        answer.SecondFinish = "■";
                    }
                    else if (answer.StatusCode == "S4")
                    {
                        answer.ThirdFinish = "■";
                    }
                    else
        ...
        private void btnSecondRecheckFinish_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            if (this.UserInfoDto.RoleType == "S")
            {
                ...
                {
                    AnswerStatusTypeCodeDto answer = grvRecheckProcess.GetRow(grvRecheckProcess.FocusedRowHandle) as AnswerStatusTypeCodeDto;
                    service.RechekComplete(CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString(), answer.ShopCode, "S3", this.UserInfoDto.UserID);
                }
            }
            SearchAnswerLog();
        }
        ...
        private void btnThirdRecheckFinish_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            if (this.UserInfoDto.RoleType == "S")
            {
                ...
                {
                    AnswerStatusTypeCodeDto answer = grvRecheckProcess.GetRow(grvRecheckProcess.FocusedRowHandle) as AnswerStatusTypeCodeDto;
                    service.RechekComplete(CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString(), answer.ShopCode, "S4", this.UserInfoDto.UserID);
                }
            }
            SearchAnswerLog();
        }
