...
namespace XHX.View
{
    public partial class ExecuteTeamAlter : BaseForm
    {
                ...
                if (item.StatusType == 'U')
                {
                    ...
                    if (item.AgreeCheck != null)
                    {
                        ...
                        service.SaveExecuteTeamAlter(item.ProjectCode, item.ShopCode, item.SubjectCode, item.ReCheckTypeCode
                            , agreeCheck, item.AgreeReason, 
                            item.NewScore == null ? Convert.ToDecimal(item.OrgScore == "未涉及" ? Convert.ToDecimal(9999.00) : Convert.ToDecimal(item.OrgScore)) : item.NewScore,
        ...
        private void btnView_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            DataSet ds1 = service.SearchShopExamTypeByProjectCodeAndShopCode(projectCode, shopCode);
            ...
            //sub.ShowDialog();
            AnswerSubjectForm answer = new AnswerSubjectForm(projectCode, subjectCode, shopCode, shopName, order, '0', this.UserInfoDto, examType,"ExecuteTeamAlter");
            answer.ShowDialog();
        }
    }
}
