        ...
        private void SearchRecheckDtl()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    reCheckDtl.PicLossError = Convert.ToString(ds.Tables[0].Rows[i]["PicLossError"]);
                    reCheckDtl.PicLazyError = Convert.ToString(ds.Tables[0].Rows[i]["PicLazyError"]);
                    reCheckDtl.PicUploadError = Convert.ToString(ds.Tables[0].Rows[i]["PicUploadError"]);
